...
- **Unvalidated Claude JSON** — `backend/app/services/ai_service.py` uses `.setdefault()` on model output, which **silently masks malformed JSON** (a missing field becomes an empty string instead of an error). Add real schema validation (Pydantic) on AI responses before trusting them.
...
### Frontend
- **⚠️ RBAC is documented but NOT enforced.** CLAUDE.md describes a `<Can>` component and `usePermission` hook — **they do not exist in the codebase.** The permission matrix in `frontend/src/config/roles.ts` is defined but never checked; `navigation.ts` items carry a `permission` field that nothing reads. **Every role currently sees every menu item.** Real access control is backend-only today. Treat the frontend as unprotected until guards are actually built.
...
**CLAUDE.md describes features that aren't actually implemented** (notably frontend RBAC enforcement). Don't trust the docs as a spec — verify against the code. This drift is itself a symptom of the scaffold-first build process, and a reminder of why Tier 3 (tests + CI) matters: tests would have caught the claim.
